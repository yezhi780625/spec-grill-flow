...
1. **繞過偵測**：本期 merged PR 數 − retro-log 新增列數。繞過流程的人不回填 retro，但 merge 記錄騙不了人——差值 >0 即有 PR 繞過；差值持續擴大是「流程太重」的最強訊號，優先刪流程而非勸人遵守。
2. constitution 各原則**與流程條款**（熔斷、絆線、豁免、未解事項……）是否攔過違規——連續兩次檢視零攔截的條款，列為精煉或刪除候選。
