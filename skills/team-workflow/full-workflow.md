...
- **Solo 模式**（個人＋AI）：這些角色由 **fresh-context 的獨立 AI agent** 代位。關鍵不是換個名字，而是換個脈絡——代位 agent **不得共享產生受審產物的對話脈絡**（撰寫 spec、實作程式的過程對話），但**可自由讀取 repo**：codebase 認識是審查的合法輸入，該擋的只有附和傾向，不是知識。prompt 明確指定挑錯立場。參與過推理的 agent 會順著自己的假設附和，等於沒審。
...
- **Solo 模式**：由 fresh-context 的獨立 subagent 執行 grill——受審對象是 spec 文件本身，不帶 Phase 1 的對話脈絡（repo 與事實查證照常可用），prompt 明確指定 adversarial 立場；作者本人作答。
...
3. 作者有權駁回或跳過任何追問，但**被駁回的追問由 grill 執行者記入 spec 的「未解事項」段——不得刪除，Phase 5 reviewer 會檢視**。跳過從無聲無息變成留痕到驗收；這是 solo 模式裡「同事皺眉頭」的替代品（solo 模式的攔截力天生弱於 team 模式，此留痕是主要補償機制，retro 時留意其症狀）。
...
**負責人**：PR reviewer——**不得是實作者本人**。Solo 模式：由 fresh-context 的獨立 agent 代位，**可自由讀取 repo**（架構偏離、效能退化、與既有模組的互動都需要 codebase 脈絡），僅不得共享實作過程的對話記錄。
...
- **同一 feature 累計退回 Phase 2 達 2 次** → 熔斷：停止流程，**強制拆分**——已想清楚的部分保留成獨立 spec 照常前進，只把糾纏不清的部分回 Phase 1 重新定義。與升級規則同一哲學：成本是重新組織，不是作廢；探索性需求退回兩次很正常，熔斷懲罰的是糾纏，不是探索。Team 模式：拆分決策拉高為專案級討論；Solo 模式：換 fresh agent 從**問題定義**（而非現有 spec）主導拆分。retro-log 症狀分類記「熔斷」。
...
1. **繞過偵測**：本期 merged PR 數 − retro-log 新增列數。分子只計 feature branch 的 merge PR，排除自動化（dependabot 等）、docs-only、revert、CI 修正——否則第一次檢視就會看到一個沒人知道該不該緊張的數字。繞過流程的人不回填 retro，但 merge 記錄騙不了人——差值 >0 即有 PR 繞過；差值持續擴大是「流程太重」的最強訊號，優先刪流程而非勸人遵守。
